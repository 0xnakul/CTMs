...
from models.modules import ParityBackbone, SynapseUNET, Squeeze, SuperLinear, Linear, LearnableFourierPositionalEncoding, MultiLearnableFourierPositionalEncoding, CustomRotationalEmbedding, CustomRotationalEmbedding1D, ShallowWide
...
class ContinuousThoughtMachine2(nn.Module, PyTorchModelHubMixin):
    """
    ...
    Args:
        ...
        do_layernorm_nlm (bool): Apply L
        ayerNorm within NLMs.
    ...
    def __init__(self,
        ...
        super(ContinuousThoughtMachine2, self).__init__()
        ...
        # --- Core CTM Modules ---
        # each group should have independed synapse model
        self.synapses = nn.ModuleList([self.get_synapses(synapse_depth, d_model, dropout) for _ in range(self.n_neuron_groups)])
        # each group shares one trace processor, significant deviation from the original CTM
        self.trace_processor = nn.ModuleList([self.get_neuron_level_models(deep_nlms, do_layernorm_nlm, memory_length, memory_hidden_dims, d_model, dropout_nlm) for _ in range(self.n_neuron_groups)])

        #  --- Start States ---
        self.register_parameter('start_activated_state', nn.Parameter(torch.zeros((self.n_neuron_groups, d_model)).uniform_(-math.sqrt(1/(d_model)), math.sqrt(1/(d_model)))))
        self.register_parameter('start_trace', nn.Parameter(torch.zeros((self.n_neuron_groups, d_model, memory_length)).uniform_(-math.sqrt(1/(d_model)), math.sqrt(1/(d_model)))))
        # for when a particular group remains inactive, it should still have a trace (pre-activation) to indicate it was inactive, it should be learnable
        self.register_parameter('inactive_pre_activation', nn.Parameter(torch.zeros((d_model)).uniform_(-math.sqrt(1/(d_model)), math.sqrt(1/(d_model)))))
    ...
    def get_neuron_level_models(self, deep_nlms, do_layernorm_nlm, memory_length, memory_hidden_dims, d_model, dropout):
        ...
        # 
        CTM2 doesn't do this. Single NLM per group 
        """
        if deep_nlms:
            return nn.Sequential(
                nn.Sequential(
                    Linear(in_dims=memory_length, out_dims=2 * memory_hidden_dims, N=d_model,
                                do_norm=do_layernorm_nlm, dropout=dropout),
                    nn.GLU(),
                    Linear(in_dims=memory_hidden_dims, out_dims=2, N=d_model,
                                do_norm=do_layernorm_nlm, dropout=dropout),
                    nn.GLU(),
                    Squeeze(-1)
                )
            ...
            return nn.Sequential(
                nn.Sequential(
                    Linear(in_dims=memory_length, out_dims=2, N=d_model,
                                do_norm=do_layernorm_nlm, dropout=dropout),
                    nn.GLU(),
                    Squeeze(-1)
                )
    ...
    def set_synchronisation_parameters(self, synch_type: str, n_synch: int, n_random_pairing_self: int = 0):
            ...
            2. Set the parameters for learnable exponential decay when computing synchronisation between all 
                neurons.
            3. CTM2: in v0, we keep each decay param separate for each groups, since different neurons in different groups 
                may sync differently. But one simplification we are going for in v0 is that we compute sync across same indices of neurons in each group.
                This can be made more complex after initial experiments.
            """
            ...
            self.register_parameter(f'decay_params_{synch_type}', nn.Parameter(torch.zeros(self.n_neuron_groups, synch_representation_size), requires_grad=True))
    ...
    def process_synapses(self, pre_synapse_input, active_groups_indices):
        B = pre_synapse_input.shape[0]
        sorted_idx = active_groups_indices.argsort()
        sorted_expert_idx = active_groups_indices[sorted_idx]
        sorted_pre_synapse_input = pre_synapse_input[sorted_idx]
        state = torch.empty((B, self.d_model), device=pre_synapse_input.device)
        for i in range(self.n_neuron_groups):
            mask = sorted_expert_idx == i
            if mask.any():
                state[mask] = self.synapses[i](sorted_pre_synapse_input[mask])
        
        _, unsort_idx = sorted_idx.sort()


        return state[unsort_idx]
    
    def process_trace(self, trace, active_groups_indices):
        """
        Process the trace for each group
        trace: (B, N_groups, H, T)
        active_groups_indices: (B,)
        process through the trace processor for each group from self.trace_processor
        """
        B, N, H, T = trace.shape
        device = trace.device
        
        # Step 1: Select each sample's active group trace → (B, H, T)
        batch_idx = torch.arange(B, device=device)
        selected_trace = trace[batch_idx, active_groups_indices]  # (B, H, T)
    
        # Step 2: Route through processors (same pattern as before)
        sorted_idx = active_groups_indices.argsort()
        sorted_expert_idx = active_groups_indices[sorted_idx]
        sorted_trace = selected_trace[sorted_idx]
        
        # Assuming output shape matches input (adjust if different)
        output = torch.empty(B, H, device=device)
        for i in range(self.n_neuron_groups):
            mask = sorted_expert_idx == i
            if mask.any():
                output[mask] = self.trace_processor[i](sorted_trace[mask])
    ...
    def forward(self, x, track=False):
        ...
        neuron_group_indices = torch.arange(0, self.n_neuron_groups, dtype=torch.int, device=device).unsqueeze(-1)
        ...
        curr_active_neuron_group = torch.zeros((B,), dtype=torch.int64, device=device)
        curr_active_neuron_group_onehot = F.one_hot(curr_active_neuron_group, num_classes=self.n_neuron_groups).float()

        # For the first tick, we start with the 1st set (0-indexed) set of neurons
        # --- Initialise Recurrent State ---
        state_trace = self.start_trace.unsqueeze(0).expand(B, -1, -1, -1) # Shape: (B, N_groups, H, T)
        activated_state = self.start_activated_state.unsqueeze(0).expand(B, -1, -1).clone()
        ...
        # NOTE: decay params are unique to each neuron set too
        ...
        r_action, r_out = torch.exp(-self.decay_params_action).unsqueeze(0).expand(B, -1, -1), torch.exp(-self.decay_params_out).unsqueeze(0).expand(B, -1, -1)
        

        # compute initial alpha and beta outn for every group
        _, decay_alpha_out_per_group, decay_beta_out_per_group = self.compute_synchronisation(activated_state[0], None, None, r_out[0], synch_type='out')
        _, decay_alpha_action_per_group, decay_beta_action_per_group = self.compute_synchronisation(activated_state[0], None, None, r_action[0], synch_type='action')

        decay_alpha_out_per_group = decay_alpha_out_per_group.unsqueeze(0).expand(B, -1, -1).clone()
        decay_beta_out_per_group = decay_beta_out_per_group.unsqueeze(0).expand(B, -1, -1).clone()
        decay_alpha_action_per_group = decay_alpha_action_per_group.unsqueeze(0).expand(B, -1, -1).clone()
        decay_beta_action_per_group = decay_beta_action_per_group.unsqueeze(0).expand(B, -1, -1).clone()

        # --- Recurrent Loop  ---
        for stepi in range(self.iterations):
            # sync func takes r_action and r_out of shape (B, 1)
            # decay_alpha and beta are not learnable, but directly derived from r and prev vals of them

            curr_activated_state = torch.einsum('bn, bnd->bd', curr_active_neuron_group_onehot, activated_state)
            curr_r_out = torch.einsum('bn, bnx->bx', curr_active_neuron_group_onehot, r_out)
            curr_r_action = torch.einsum('bn, bnx->bx', curr_active_neuron_group_onehot, r_action)
            
            if stepi == 0:
                _, decay_alpha_out, decay_beta_out = self.compute_synchronisation(curr_activated_state, None, None, curr_r_out, synch_type='out')
                # Compute learned weighting for synchronisation
            else:
                decay_alpha_out = torch.einsum('bn, bnx->bx', curr_active_neuron_group_onehot, decay_alpha_out_per_group)
                decay_beta_out = torch.einsum('bn, bnx->bx', curr_active_neuron_group_onehot, decay_beta_out_per_group)

            # --- Calculate Synchronisation for Input Data Interaction ---
            if stepi == 0:
                decay_alpha_action, decay_beta_action = None, None      # to conform with original implementation
            else:
                decay_alpha_action = torch.einsum('bn, bnx->bx', curr_active_neuron_group_onehot, decay_alpha_action_per_group)
                decay_beta_action = torch.einsum('bn, bnx->bx', curr_active_neuron_group_onehot, decay_beta_action_per_group)
    
            synchronisation_action, decay_alpha_action, decay_beta_action = self.compute_synchronisation(curr_activated_state, decay_alpha_action, decay_beta_action, curr_r_action, synch_type='action')

            # --- Interact with Data via Attention ---
            q = self.q_proj(synchronisation_action).unsqueeze(1)
            attn_out, attn_weights = self.attention(q, kv, kv, average_attn_weights=False, need_weights=True)
            attn_out = attn_out.squeeze(1)
            pre_synapse_input = torch.concatenate((attn_out, curr_activated_state), dim=-1)

            # --- Apply Synapses ---;;;;
            active_groups_indices = torch.argmax(curr_active_neuron_group_onehot, dim=-1)       # Shape: (B,); which group is active for each sample
            state = self.process_synapses(pre_synapse_input, active_groups_indices)
            ...
            synchronisation_out, decay_alpha_out, decay_beta_out = self.compute_synchronisation(curr_activated_state, decay_alpha_out, decay_beta_out, curr_r_out, synch_type='out')
            ...
            decay_beta_action_per_group[torch.arange(B), active_groups_indices] = decay_beta_action
