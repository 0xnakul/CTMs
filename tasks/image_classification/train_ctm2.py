...
from models.ctm2 import ContinuousThoughtMachine2
...
def parse_args():
    ...
    # Model Selection
    parser.add_argument('--model', type=str, default='ctm', choices=['ctm2', 'lstm', 'ff'], help='Model type to train.')
    ...
    parser.add_argument('--positional_embedding_type', type=str, default='none', help='Type of positional embedding (CTM, LSTM).',
                        choices=['none',
                                 ...
                                 'custom-rotational'])
    # CTM2 specific
    parser.add_argument('--n_neuron_groups', type=int, default=8, help='Number of neuron groups to use for CTM2.')
    parser.add_argument('--group_router_type', type=str, default='mlp', help='Type of group router to use for CTM2.', choices=['mlp', 'linear'])
    parser.add_argument('--use_tick_conditioned_routing', action="store_true", default=True, help='Use tick conditioned routing in CTM2.')
    parser.add_argument('--dropout_router', type=float, default=0.2, help='Dropout rate for the group router in CTM2.')
    parser.add_argument('--use_tick_training_curriculum', action="store_true", default=False, help='Use tick curriculum during training for CTM2.')
...
if __name__=='__main__':
    ...
    print(f'{args.use_tick_conditioned_routing}')
    model = None
    if args.model == 'ctm2':
        model = ContinuousThoughtMachine2(
            iterations=args.iterations,
            d_model=args.d_model,
            n_neuron_groups=args.n_neuron_groups, group_router_type=args.group_router_type, use_tick_conditioned_routing=args.use_tick_conditioned_routing, dropout_router=args.dropout_router, use_tick_training_curriculum=args.use_tick_training_curriculum,
    ...
    for _ in range(8):      # each neuron group should be activated
    ...
    train_accuracies_most_certain = [] if args.model in ['ctm2', 'lstm'] else None
    test_accuracies_most_certain = [] if args.model in ['ctm2', 'lstm'] else None
                ...
                if args.model in ['ctm2', 'lstm']:
                    try: # For safety
                        ...
                        try:
                            ...
                            print(f"Neural dynamics plotting failed for model {args.model}: {e}")
